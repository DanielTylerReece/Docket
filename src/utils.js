...
/**
 * Requests the calendar backend to create new objects. In the case of
 * task lists, this would create new tasks.
 *
 * @param {ECal.Client} client - `ECal.Client` of a source.
 * @param {ICalGLib.Component[]} obj - Components to create.
 * @param {ECal.OperationFlags} flag - bit-or of `ECal.OperationFlags`.
 * @param {Gio.Cancellable} [cancellable] - Cancellable object.
 * @returns {Promise<string[]>} UIDs of the created objects.
 */
export function createObjects_(client, obj, flag, cancellable = null) {
    return new Promise((resolve, reject) => {
        client.create_objects(obj, flag, cancellable, (self, res) => {
            try {
                resolve(self.create_objects_finish(res));
            } catch (e) {
                reject(e);
            }
        });
